...
I assume that you have a functional version of Python 3, Pip, Npm, PostgreSQL, and pipenv or similar pyhon virtual shell available on your machine.

You will also need a cloudinary account that includes an upload preset with these image transofrmation settings c_limit,cs_srgb,h_500,w_1000.

* Fork and clone the repo
* `$ pipenv shell` to createenter the Virtual shell
* `$ pipenv install` to install dependancies from pipfile. (Be patient, they're seizeable)
* `$ brew services start postgresql` to confirm postgreSQL is running
* `$ createdb image-editing-db` to create database
* `$ python3 manage.py migrate` to create default tables
* `$ python3 manage.py loaddata images/seeds.json` to add all filters to the DB
...
When we initially decided to create the image app using Scikit Image we quickly realized that we would have two major requirements that could make or break the project. Firstly how do we get the image between the user's frontend and the python image libraries without burning through our Cloudinary credits. And secondly is it possible to use MatplotLib's graph outputs as a useable PNG or JPG.

To overcome the first blocker I researched using Base 64 encoding to encode the images as a string of text so that the image could be included in a Json Response.

Secondly we prototyped using Matplot's savefig function using google Colaboratory, which is essentially a Jupyter Notebook hosted by Google. Although there were problems with the savefig function including whitespace and needing to remove graph axis, we had proof of concept and decided that we would be able to overcome these issues.
...
All of the filters are unique in how they achieve their output. Commented examples below are of the Tint and Histogram Filters.

### Tint Filter
The tint filter takes the image, recalculates the image to grayscale, calculates where soft and hard gradients exist within the image and then re-renders the image in the user's chosen colorspace.
```python
...
# rgb2gray takes the 3D array created by io.imread and calculates the luminance of each pixel. It then returns a 2D array containing only the luminance of each pixel
...
# This checks whether the filter is being applied to a user's image or to a thumbnail. Producing smaller images imporoves algorithmic efficiency for both memory and processing.
if thumbnail == True:
    size = (1, 1)
else:
    size = (8, 8)

# Checks whether the user's image or reference image pixel data includes an Alpha channel. If it does it removes the alpha channel using Alpha Compositing.
print(image.shape)
if image.shape[2] == 4:
...
# Saves the image as a temporary file with a unique filename depending on whether the image has been previously saved to filesystem or is a URL
if path[:4] == 'http':
...
A blocker that I personally struggled to overcome was outputting an base 64 image from the canvas context. I expected that this was due to my inexperience using Hooks in React . I reached out to Anton Lavrenov who created the Konva library and he helped shed some light on what I needed to fix.
