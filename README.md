...
* `$ pipenv install` to install dependancies from pipfile. (Be patient, they're sizeable)
...
#### Tint Filter
The tint filter takes the image, recalculates the image to grayscale, calculates where soft and hard gradients exist within the image and then re-renders the image in the user's chosen colorspace. Copy and paste it into a Colab or Jupyter notebook and have a play with the options. change the image URL to anotehr image and try changing color_space to 'blues', 'copper', or 'pink_r'
...
# Producing smaller images imporoves algorithmic efficiency for both memory and processing.
if thumbnail == True:
    size = (1, 1)
else:
    size = (8, 8)
...
## Key Takeaways
* HTML5 Canvas is an incredibly powerful element, but dificult to use in React. Thankfully tools such as Konva-React make it a lot easier to use.
...
* I learnt a lot about using 2D and 3D arrays in python. SciKit, pillow, MatPlotLib
